...
def checker_coord(*args) -> None:
    if hasattr(args[0], "append"):
        line:list[int, int, int]|list[int, int]                = args[0]
        limit:list[int, int, int]|list[int, int]|Literal["ex"] = args[1]
        ...
        node_base:BasisNode = args[0]
        node_to  :BasisNode = args[1]
        ...
        for nodes in node_base.child_lst:
            ...
            #print(nodes.vec, node_to.vec)
            if nodes.vec == node_to.vec:
                raise CoordInConflic(nodes, node_to)
